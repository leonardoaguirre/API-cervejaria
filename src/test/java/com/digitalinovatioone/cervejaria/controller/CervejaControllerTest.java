...
public class CervejaControllerTest {
    ...
    @Test
    void quandoPATCHChamadoParaDecrementarEntaoRetornaStatusOK() throws Exception {
        ...
        when(cervejaService.decrementar(ID_CERVEJA_VALIDO, quantidadeDTO.getQuantidade())).thenReturn(cervejaDTO);

        mockMvc.perform(patch(CERVEJA_API_URL + "/" + ID_CERVEJA_VALIDO + CERVEJA_API_URL_SUBPATH_DECREMENT)
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    @Test
    void quandoPATCHChamadoParaDecrementarMenorQueZeroEntaoRetornaStatusBadRequest() throws Exception {
        QuantidadeDTO quantidadeDTO = QuantidadeDTO.builder()
                .quantidade(60)
                .build();
        ...
        when(cervejaService.decrementar(ID_CERVEJA_VALIDO, quantidadeDTO.getQuantidade())).thenThrow(EstoqueDeBebidaExcedidoException.class);
    ...
    @Test
    void quandoPATCHChamadoComIdCervejaInvalidoPataDecrementarEntaoRetornaStatusNotFound() throws Exception {
        QuantidadeDTO quantidadeDTO = QuantidadeDTO.builder()
                .quantidade(5)
                .build();

        when(cervejaService.decrementar(ID_CERVEJA_INVALIDO, quantidadeDTO.getQuantidade())).thenThrow(BebidaNaoExisteException.class);
        mockMvc.perform(patch(CERVEJA_API_URL + "/" + ID_CERVEJA_INVALIDO + CERVEJA_API_URL_SUBPATH_DECREMENT)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(asJsonString(quantidadeDTO)))
                .andExpect(status().isNotFound());
    }
